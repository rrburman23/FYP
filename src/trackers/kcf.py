...
class KCFTracker(TrackerBase):
    def __init__(self):
        # Check if KCF tracker is available in the current OpenCV installation
        if hasattr(cv2, 'legacy') and hasattr(cv2.legacy, 'TrackerKCF_create'):
            self.tracker = cv2.legacy.TrackerKCF_create()  # Use legacy version if available
        elif hasattr(cv2, 'TrackerKCF_create'):
            self.tracker = cv2.TrackerKCF_create()  # Use non-legacy version
        else:
